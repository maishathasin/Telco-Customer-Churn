...
Model & Accuracy & F1 label=0 & F1 label=1 & Hyperparameters \\ \hline
Decision Tree & 0.81 & 0.88 & 0.58 & $\alpha$ = 0.000731 \\
Decision Tree + SMOTE & 0.82 & 0.88 & 0.65 & $\alpha$ = 0.000897 \\
Random Forest & 0.83 & 0.89 & 0.62 & m = sqrt, max depth=8 \\
Random Forest + SMOTE & 0.83 & 0.88 & 0.65 & m = sqrt, max depth=11 \\
Single-Layer Perceptron & 0.84 & 0.89 & 0.66 & activation = logistic, solver = adam, \\
&&&& hidden layers = (5,) \\
Multi-Layer Perceptron & 0.83 & 0.89 & 0.65 & activation = logistic, solver = adam, \\
&&&& hidden layers = (5, 5, 5) \\
\hline
\end{tabular}
...
- Talk about specific models here

For the Decision Tree model, we can see that SMOTE results in an increased value of $\alpha$. This indicates that even with k-fold CV, the base Decision Tree model might still be overfitting somewhat, as the Decision Tree with SMOTE model performs better overall with a smaller subtree. It may be that SMOTE is only countering the tendency to classify towards the majority class and that just so happens to be with a larger penalty, as it is difficult to draw a causal link between the two.
