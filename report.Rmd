...
- Customers with month-month contracts are likely to churn, this is obvious as the customer is not committed so there is a direct correlation between churn and contract. The same goes for Tech support and Device protection dimensions
...
-  From Figure 7 we can see that there are no clear outliers in the CLTV, this is good because we can use classification methods like KNN, Logistic regression without worrying too much about the outliers causing the data to over fit.  
...
## KNN
In our implementation of the K-Nearest Neighbors (KNN) algorithm, we employed the GridSearchCV method again  to determine the optimal hyperparameters . Specifically, we tuned the number of neighbors, considering various distance metrics such as Minkowski, Euclidean, Manhattan, and Cosine. The Minkowski distance metric is a generalized version of Manhattan and Euclidean distances in a vector norm space.[@Cunningham_2021]
...
## Concluding Results 
From our model we can see that most models are almost give the same accuracy and complex models are not needed for such a dataset. In terms of performance on other bootstrapped data, computational complexity and intelligibility I would fit this dataset with the SVM model, although it is slower than Logistic regression, it performs better on 'unseen' data than LR also, it's its less complex and computationally intensive than trees and Neural Networks, for the same accuracy and fair levels of F1.  
...
=======
## Future research
>>>>>>> 0b2c606116e99ed4a2e91268fe88e072be54458d
...
![EDA](cont_charges.png)

![Showing where most customers are churning from, a large number of customers from LA](country_usa.png)
![Figure 9:Showing the number of features selected vs the Cross validation score](features.png)
